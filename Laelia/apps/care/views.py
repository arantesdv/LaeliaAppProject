...
def index_view(request):
	user = request.user
	professional = professional_model.objects.get(user=user)
...
class EventUpdate(UpdateView):
	...
	def get_context_data(self, **kwargs):
		...
		context['event'] = TimeLineEvent.objects.get(pk=self.kwargs['event_id'])
		return context
...
class EventList(ListView):
	...
	def get_queryset(self):
		...
		return TimeLineEvent.objects.filter(relation_id=relation.id)
